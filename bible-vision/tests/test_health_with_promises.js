...
function makeResolvedValue(value) {
  ...
  return (new Promise((resolve, reject) => {resolve(value)})); // TODO: replace this line
}
...
async function doubleAfterShortDelay(n) {
  // TODO Q3:
  await wait(10);
  return (n*2); // TODO: replace this line
}
...
function sumSquaresParallel(nums) {
  ...
  return (Promise.all(ary).then(outcome => outcome.reduce((sum, n) => sum + n, 0))); // TODO: replace this line
}
...
function collectSuccessfulValues(promises) {
  ...
  return (Promise.allSettled(promises).then(outcome => outcome.filter((p) => p.status === "fulfilled").map((pr) => pr.value)));
...
async function fetchHealthTwiceInParallel(request) {
  // TODO Q7:
  //   1. Call apiHealth(request) twice WITHOUT awaiting immediately (two Promises).
  //   2. Use Promise.all([...]) to await both.
  //   3. Return the resulting array.
  let firstPromise = apiHealth(request);
  let secondPromise = apiHealth(request);
  return  (Promise.all([firstPromise, secondPromise]));
  //my attempt, not working becuase apiHealth is not working, also probably bugs with this at the moment
}
